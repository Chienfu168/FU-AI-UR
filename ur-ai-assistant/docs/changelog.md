...
v1.27.0 上線後，使用者詢問「文稿有無最低量的設定，避免文案內容太
少」。當時的檢查只擋「完全空白」的問題／回答，內容再怎麼簡略（例如
只有一句話）都會直接送給 AI 擴寫，AI 為了湊出約 500～900 字的文章
長度，容易自行「加油添醋」補上 FAQ 沒提到的細節，違背這個功能「只
根據 FAQ 內容延伸、不捏造」的設計原則。這一版新增最低字數門檻。

一、主要新增

1. `UR_AI_FAQ_Article_Service` 新增常數 `MIN_SOURCE_LENGTH = 120`
   與私有方法 `source_length()`：計算「問題＋回答」去除 HTML 標籤
   後的總字數（使用 `mb_strlen()` 正確處理中文字元），低於 120 字
   時直接擋下、不呼叫 AI，回傳明確錯誤訊息，建議管理者先補充這則
   FAQ 的回答內容再嘗試產生文章。
2. 120 字的門檻是依實際 FAQ 內容包（`data/industry-packs/
   land_agent/faq.csv`）的字數分布估算：問題＋回答加總最短一筆
   133 字、中位數 159 字，120 字明顯低於這個中位數，不會誤擋正常
   填寫的 FAQ，但足以濾掉真正過於簡略、不足以支撐一篇文章的內容。

二、設計原則

- 門檻值先寫死一個合理預設，不做成「功能設定」頁的可調整選項：
  這只是一道防呆保護，不是核心業務邏輯，不同管理者對「多短算太
  短」的判斷差異不大，先用簡單方式處理，之後如果真的需要因產業
  別或使用情境調整，再改成設定項也不遲。
- 字數檢查在呼叫 AI 之前執行（而非等 AI 回傳後才檢查內容品質），
  內容不足時完全不會產生 API 呼叫費用。

三、測試方式

PHP 單元測試：`source_length()` 正確加總問題＋回答的中文字數；
問題＋回答加總低於 120 字時，`create_from_faq()` 在呼叫 AI 前就
正確短路並回傳提及門檻字數的錯誤訊息；達到門檻的正常內容不受影響
（沿用 v1.27.0 既有的成功產生測試，並將測試用 FAQ 內容加長至符合
新門檻，確認原本情境仍正確通過）。全部情境通過，`php -l` 確認
語法正確。

四、是否建議上正式站

建議更新。這是既有功能的防呆強化，不影響任何其他既有行為；唯一
的行為變化是「問題＋回答加總不足 120 字的 FAQ」按下「產生文章
草稿」時，會改為顯示提示訊息而非直接呼叫 AI。
...
v1.27.1 加了「來源內容太薄」的防呆後，使用者接著提出：「產生的
文案，也要有品質的控管；文案需要有一定內容，才能成為好文案，並且
要包含建議分類與標籤」。這一版補上兩塊還沒處理到的品質控管：AI
「產出」的文章內容本身夠不夠長（v1.27.1 只管了「輸入」的 FAQ 內容
夠不夠長），以及文章缺少分類與標籤、不利於網站內容管理與 SEO 的
問題。

一、主要新增

1. `UR_AI_FAQ_Article_Service` 新增常數 `MIN_ARTICLE_LENGTH = 300`：
   AI 產生的文章內文（去除 HTML 標籤後）不足 300 字時，視為品質
   不佳（可能沒有依系統提示詞要求確實展開到約 500～900 字），直接
   擋下、不建立文章草稿，回傳提示請重新嘗試——與其把品質不佳的
   草稿也建立出來，讓管理者事後才發現內容太薄，不如在建立前就擋
   下來，讓管理者可以直接重新按一次「產生文章草稿」。
2. 新增 `UR_AI_FAQ_Article_Service::suggest_category()`／
   `suggest_keywords()`：沿用既有的 `UR_AI_FAQ_Category_Helper`
   （v1.25.0 已改為依產業別調整分類規則與關鍵字候選清單），依「來源
   FAQ 的問題＋回答」（而非 AI 寫出來的文章內容）建議這篇文章的
   分類與標籤——用已經人工審核過的 FAQ 內容判斷分類，比用 AI 生成
   的文章內容判斷更穩定可靠。
3. 新增 `resolve_category_id()`：把建議的分類名稱轉換成 WordPress
   分類（`category` 分類法）的 term_id，找不到現有分類時會自動
   建立一個新分類，下次遇到同名分類會直接沿用、不會重複建立。若
   分類工具判斷不出適合分類（回傳「待分類」），則不指定分類，讓
   文章沿用 WordPress 預設分類（通常是「未分類」），不會為了「找
   不到分類」這種情況硬建一個叫「待分類」的分類，避免分類清單裡
   出現沒有意義的分類。
4. `wp_insert_post()` 的參數新增 `post_category`（上述分類
   term_id）與 `tags_input`（建議關鍵字轉成的標籤陣列），成功時
   一併回傳 `category`／`keywords` 欄位；AJAX 成功訊息也會顯示
   「已產生文章草稿（分類：OO／標籤：AA, BB），請於文章編輯畫面
   核對內容後再發布。」，管理者不用點進文章編輯畫面就能先確認 AI
   建議的分類標籤是否合理。

二、設計原則

- 內容長度門檻延續 v1.27.1「輸入內容過薄」防呆的同一套邏輯與
  精神，差別只是這次管的是「AI 產出的內容」而不是「FAQ 原始內容」；
  兩個門檻獨立檢查，互不影響。
- 分類／標籤建議刻意沿用既有的 `UR_AI_FAQ_Category_Helper`，不是
  另外做一套「文章專用」的分類判斷邏輯——這個工具已經是依產業別
  調整過的成熟邏輯（v1.25.0），沒有理由重新發明一遍；也代表文章
  的分類與這則 FAQ 在「FAQ 知識庫」頁看到的分類建議會是一致的。
- 找不到分類時完全不指定分類（沿用 WordPress 預設），不強制建立
  或指定一個沒有實質意義的分類，避免污染網站既有的分類清單。

三、測試方式

PHP 單元測試（沿用同一個驗證腳本，新增／擴充多項情境）：AI 產生
內容不足 300 字時，`create_from_faq()` 在建立文章前就正確擋下並
回傳提及門檻字數的錯誤訊息；正常情境下正確依來源 FAQ 問答建議
分類（以真實的 `UR_AI_FAQ_Category_Helper` 而非假物件驗證，確認
「自主更新」相關問答能正確判斷出「自主更新」分類、且不是依 AI
寫出來的文章內容判斷）並正確轉換成 `post_category` 的 term_id；
同一分類重複產生第二篇文章時，正確沿用既有分類 term、不會重複
建立；建議關鍵字正確轉換成 `tags_input` 陣列並實際出現在
`wp_insert_post()` 的參數中；AJAX 回應正確附上 `category`／
`keywords` 欄位，成功訊息正確附上分類與標籤內容。全部情境通過，
`php -l`／`node --check` 確認語法正確。

四、是否建議上正式站

建議更新。文章分類與標籤是新增的附加資訊，不影響既有任何頁面或
功能的行為；唯一的行為變化是「AI 產生的文章內容不足 300 字」時
會被擋下改為提示重新嘗試，以及新產生的文章草稿會多出分類與標籤。
...
使用者回報：「AI 對話後產生知識庫的內容（可轉為草稿）已經正常，但
發現會產生幾個知識庫內容，但每個的固定回答內容，都太過精簡。品質
上有些不足。」——即 v1.26.0「AI 對話」功能的「產生總結草稿」，雖然
運作正常（能正確整理出多則草稿），但草稿的「固定回答」內容常常
只有一句話帶過，不足以直接收錄成一則有實質內容的 FAQ。這一版針對
「產生總結草稿」的答案品質做兩層強化，做法與精神比照 v1.27.1／
v1.28.0 已經建立的「輸入／輸出內容品質門檻」模式。

一、主要新增

1. 加強 `admin_chat_summarize_system_prompt()`：明確要求「固定回答」
   必須是完整、具體的一段說明，至少涵蓋 2～3 句完整內容（結論＋
   原因或條件＋注意事項），不可以只用一句話簡短帶過；同時強化既有
   的「內容不足就不要勉強生成」原則，明確納入「內容過於簡略」也
   算在「不要勉強生成」的範圍內，不是只看「有沒有講到重點」。
2. `UR_AI_OpenAI_Client` 新增常數 `MIN_FAQ_DRAFT_ANSWER_LENGTH =
   60`：解析 AI 回傳的草稿清單時，逐則檢查「固定回答」字數（去除
   HTML 標籤後），不足 60 字的草稿直接捨棄、不會出現在整理結果中
   ——這是系統提示詞之外的第二層防護網，即使 AI 偶爾沒有確實照
   提示詞要求寫出完整回答，過短的草稿也不會流出到後台讓管理者
   誤收錄。捨棄後若剩餘則數變少（甚至變成 0 則），沿用既有的
   「這段對話內容還不足以整理出明確的 FAQ 草稿」提示訊息，不需要
   額外處理。
3. `max_tokens` 由 1800 調高到 2400：配合要求答案更完整的提示詞
   調整，避免最多 5 則草稿在答案變長後被總 token 上限提前截斷。

二、設計原則

- 沿用 v1.27.1／v1.28.0 已經建立的「提示詞要求＋硬性字數門檻」
  雙層防護模式：提示詞是第一層（讓 AI 一開始就盡量寫得完整），
  字數門檻是第二層安全網（防止 AI 沒有確實照做時，品質不佳的
  內容還是流出去）。
- 字數門檻設在「固定回答」單一欄位上（而不是像文章草稿那樣看
  「問題＋回答」加總），因為這裡本來就是各自獨立的一則則 FAQ
  草稿，門檻只需要確保「回答」本身夠完整即可。60 字略低於既有
  FAQ 內容包答案的一般長度，用意是抓出「一句話帶過」這種明顯
  不合格的情況，不是要求「剛好達到理想長度」。
- 捨棄過短草稿後不特別跳出「捨棄了幾則」的訊息：管理者本來就是
  看最終呈現的草稿清單逐則審核，草稿變少但每則品質都合格，比
  草稿數量對但其中夾雜劣質內容更符合這個功能「AI 只是輔助草擬，
  最終品質仍由經營者把關」的一貫原則。

三、測試方式

PHP 單元測試：混合批次（過短／剛好低於門檻／完整合格三種情境）
下，只有達到 60 字門檻的草稿會保留下來；邊界測試確認剛好 60 字
會保留、59 字會捨棄（門檻是「不低於」而非「大於」）；全部草稿都
過短時，正確回傳空的 `drafts` 陣列（成功但沒有草稿，而不是視為
錯誤）；系統提示詞內容確認已包含「2～3 句」「不可以只用一句話
簡短帶過」等新增的品質要求文字。PHP 整合測試：`UR_AI_Admin_Chat_
Ajax::handle_summarize()` 端對端只回傳合格的草稿（分類／關鍵字
建議仍正確附上），全部草稿都過短時正確回傳既有的「內容不足」
錯誤訊息。另外重新執行 v1.26.0 既有的 AJAX 整合測試（並把測試用
的過短模擬回答改為符合新門檻的完整內容），確認既有情境仍正確
通過，沒有引入回歸。全部情境通過，`php -l` 確認語法正確。

四、是否建議上正式站

建議更新。這是既有「AI 對話」功能的答案品質強化，不影響其他既有
功能；唯一的行為變化是「產生總結草稿」整理出的草稿數量可能會比
之前少（因為過短的草稿會被捨棄），但保留下來的每一則品質都更好。
...
v1.29.0 把「固定回答最低字數」寫死為 60 字，上線後使用者回饋：
「固定回答的質量可以改為後臺可以調整的設定嗎？我個人覺得還是不夠，
但可能會視不同標準。」——不同網站經營者對「多長才算內容充實」的
標準本來就會不一樣，寫死一個數字沒辦法讓每個人都滿意，這一版把這
個門檻改為「功能設定」頁可自行調整的設定，不用改程式碼就能依自己
的標準調整。

一、主要新增

1. `UR_AI_Settings` 新增設定鍵 `admin_chat_min_draft_answer_length`
   （預設 60，可調整範圍 20～500 字）與對應的 `get_admin_chat_min_
   draft_answer_length()` 讀取方法，比照既有 `max_question_length`
   等數字型設定的寫法（含 `defaults()`、`sanitize_value()` 的上下限
   收斂）。
2. 「功能設定」頁「使用限制與成本控管」卡片後新增一張「AI 輔助
   內容品質控管」卡片，目前包含「AI 對話草稿最低回答字數」欄位，
   附上完整的欄位說明文字。
3. `UR_AI_OpenAI_Client` 移除原本寫死的 `MIN_FAQ_DRAFT_ANSWER_LENGTH`
   常數，改為 `get_min_faq_draft_answer_length()`（讀取上述設定，
   讀不到時退回預設 60，行為與升級前完全相同）；`admin_chat_
   summarize_system_prompt()` 也改為動態讀取目前設定的字數，把
   「固定回答」的要求文字（例如「至少 100 字以上」）跟著設定值走，
   而不是提示詞寫一個數字、實際門檻又是另一個數字。

二、設計原則

- 這是回應使用者明確提出的「希望可調整」需求，而不是自行猜測門檻
  該不該可調整；比照先前處理「文章草稿最低字數」時的討論（使用者
  當時選擇維持寫死），這次使用者的判斷剛好相反，兩個功能各自依
  使用者實際的選擇處理，不強求整個外掛的所有字數門檻都用同一種
  做法。
- 預設值維持 60 字（v1.29.0 上線時的原始水準），確保沒有特別調整
  過的網站，升級後行為完全不變，只有選擇調整過設定的網站才會感受
  到差異。
- 系統提示詞動態帶入設定值，而不是維持寫死的「2～3 句」加上另外
  獨立的字數門檻各自運作——讓 AI 收到的指示與實際會被執行的門檻
  一致，避免「提示詞說一套、程式碼把關又是另一套」的不一致情況。

三、測試方式

PHP 單元測試（使用真實的 `UR_AI_Settings` 類別搭配記憶體內模擬的
`get_option()`／`update_option()`，而非假物件）：預設值為 60；
`update()` 可正確寫入並讀回自訂數值；低於 20 會收斂到 20、高於
500 會收斂到 500。PHP 整合測試：以真實的 `UR_AI_OpenAI_Client`
搭配不同長度（45／90／140 字）的模擬草稿回應，確認調整設定值後
（60→100→30）門檻確實跟著變動、正確篩選出對應長度的草稿；系統
提示詞內容確認會動態反映目前設定的字數。另外重新執行 v1.26.0／
v1.29.0 既有的 AJAX 整合測試（假物件版的 `UR_AI_Settings` 補上新
方法，回傳預設值 60），確認既有情境沒有引入回歸。全部情境通過，
`php -l` 確認語法正確。

四、是否建議上正式站

建議更新。這是既有品質門檻從「寫死」改為「可調整設定」，預設值
不變，未調整過設定的網站升級後行為完全相同；想要更嚴格或更寬鬆
標準的網站，現在可以直接到「功能設定」頁調整，不需要改程式碼。
...
使用者討論外掛未來「免費／付費」的發展方向後，確認採用「服務加值」
而非「程式碼加解鎖」的模式——外掛本身（含所有現有功能）維持完全
免費開源，不做任何鎖碼判斷；未來若要推出「代管 AI 服務」（使用者
不用自己申請 OpenAI API Key，改用經營者代管的服務，依訂閱或用量
計費），也應該讓使用者自行選擇是否要切換，而不是綁死在程式碼裡。

規劃出的架構是：獨立網站（例如 ur-promoter.com）安裝 WooCommerce
＋訂閱外掛＋金流串接負責訂單與月租扣款，並在同一個網站上開一個
REST API 端點做 AI 代理服務；外掛端只需要能夠「切換要呼叫 OpenAI
官方端點，還是呼叫代管服務端點」。這一版只做外掛端這一小塊——
確認架構後才動手，避免因為商業模式細節沒想清楚而事後又要回頭
修改。

一、主要新增

1. `UR_AI_Settings` 新增三個設定鍵：`ai_service_mode`（`self`／
   `hosted`，預設 `self`）、`hosted_service_endpoint`（代管服務
   端點網址，預設空白）、`hosted_service_token`（代管服務授權碼，
   預設空白），與對應的 `get_ai_service_mode()`／
   `is_hosted_ai_service_enabled()`／`get_hosted_service_endpoint()`／
   `get_hosted_service_token()` 讀取方法。無效的 `ai_service_mode`
   值會收斂回預設的 `self`，不會被原樣存入。
2. `UR_AI_OpenAI_Client` 新增 `get_api_endpoint()`，並把原本呼叫
   五個 AI 功能（問答、AI 對話、產生總結草稿、知識大考驗出題、
   文章草稿）共用的 `self::API_ENDPOINT` 常數，全部改為呼叫這個
   方法：預設（`self` 模式）回傳與升級前完全相同的 OpenAI 官方
   端點；`hosted` 模式且已填寫端點網址時，改回傳代管服務端點；
   `hosted` 模式但端點網址空白時，安全退回 OpenAI 官方端點，不會
   呼叫到空網址。既有的 `get_api_key()` 也比照調整：`hosted` 模式
   下改回傳代管服務的授權碼，取代原本的 OpenAI API Key，做為
   Authorization header 的 Bearer 憑證。
3. 「功能設定」頁「AI API 設定」卡片，OpenAI API Key 欄位下方新增
   「AI 服務來源」下拉選單（預設「自行提供 OpenAI API Key」）與
   「代管服務端點網址」「代管服務授權碼」兩個欄位（僅在選擇「使用
   代管服務」時才會生效）。

二、設計原則

- 只做「確認會用到、且不管最終商業模式怎麼設計都用得到」的這一
  小塊技術基礎（可切換的端點／憑證），不預先做授權碼格式驗證、
  用量計費等更深入的機制——那些屬於代管服務本體（另一個獨立
  網站／專案）的範疇，商業細節（怎麼收費、要不要依站數或用量
  計費）還沒確定前先不動，避免做了又要因為模式調整而重做。
- 預設值（`self`）與既有五個 AI 功能呼叫端點／憑證的解析方式
  完全不變，確保沒有選用代管服務的網站，升級後行為與升級前一模
  一樣。
- 端點與憑證的解析邏輯集中在 `get_api_endpoint()`／`get_api_key()`
  兩個既有方法內部調整，而不是在五個呼叫端各自加判斷式——沿用
  「AI 對話」等既有功能同樣的『改一個地方、所有呼叫端自動生效』
  設計方式。

三、測試方式

PHP 整合測試（使用真實的 `UR_AI_Settings` 與 `UR_AI_OpenAI_Client`，
攔截 `wp_remote_post()` 記錄實際呼叫的網址與 Authorization
header）：預設模式下，呼叫網址與 Bearer 憑證與升級前完全相同
（即以 OpenAI 官方端點＋自行填寫的 API Key 呼叫）；切換為代管
模式並填入端點與授權碼後，呼叫確實改打代管端點、帶上代管授權碼
（不是 OpenAI 端點或 Key）；代管模式但端點空白時，安全退回 OpenAI
官方端點；切回自行提供模式後，行為立即恢復。另外重新執行既有的
文章草稿／AI 對話相關 PHP 驗證腳本，確認沒有引入回歸。全部情境
通過，`php -l` 確認語法正確。

四、是否建議上正式站

建議更新。這一版只新增一個預設不啟用的設定選項，五個既有 AI 功能
的呼叫方式對所有未調整過設定的網站完全不受影響；代管服務本體
（獨立網站上的訂單管理與 AI 代理服務）尚未實作，這個設定目前只是
為未來鋪路，暫時不會有任何網站真正切換到「使用代管服務」。
...
使用者詢問「FAQ 所生成的文章，分類是否可以多個，以文章的建議增加
分類？」——v1.28.0 上線的「產生文章草稿」功能，分類建議只會取比對
分數最高的單一分類，分數較低但也確實比對到關鍵字的其他分類會被
捨棄。使用者確認希望改成可以同時有多個分類，數量抓 3～5 個之間，
這一版把上限訂為 5。

一、主要新增

1. `UR_AI_FAQ_Category_Helper` 新增 `suggest_categories($question,
   $answer, $max = 5)`（複數版）：沿用既有 `suggest_category()` 完全
   相同的關鍵字比對與計分邏輯，差別是不再只保留分數最高的單一分類，
   而是把所有分數 > 0 的分類依分數由高到低排序後回傳，並依 `$max`
   參數截斷（預設 5）。比對不到任何分類時，仍回傳單一元素的
   `array('待分類')`，與既有單數版行為一致。既有的 `suggest_
   category()`（單數版，供 FAQ 編輯、AI 對話草稿、熱門問題、相關
   頁面等既有功能使用）完全沒有修改，不受影響。
2. `UR_AI_FAQ_Article_Service` 新增常數 `MAX_CATEGORIES = 5`，改用
   上述複數版方法取得多個建議分類，並新增 `resolve_category_ids()`
   把每個分類名稱各自轉換成 WordPress 分類 term_id（沿用既有
   `resolve_category_id()` 逐一呼叫，找不到的分類自動建立、已存在
   的分類直接沿用，過濾掉「待分類」與重複的 term_id），全部指定為
   文章的 `post_category`。
3. `UR_AI_FAQ_Ajax` 的成功回應與訊息組成，從 `category`（單一字串）
   改為 `categories`（陣列），成功訊息會用頓號列出全部套用的分類
   （例如「分類：自主更新、都市更新」），讓管理者一次看到所有套用
   結果。

二、設計原則

- 判斷依據維持不變：沿用「來源 FAQ 問題＋回答」的內容判斷（跟
  v1.28.0 一樣），不是分析 AI 產生的文章內文——文章內文是 AI
  生成的，用它來判斷分類不如用已經人工審核過的 FAQ 內容可靠，這個
  原則在這次擴充成多分類時延續，沒有改變。
- 設上限（5）而不是「所有分數 > 0 的分類全部保留」，避免關鍵字比對
  比較寬鬆時，一次掛上一堆關聯度其實不高的分類，讓分類清單失去
  篩選內容的意義；上限值可再依實際使用狀況調整，目前先抓使用者
  建議範圍（3～5）的上緣。
- 只新增複數版方法，不修改既有的單數版 `suggest_category()`——FAQ
  編輯頁、AI 對話草稿、熱門問題、相關頁面等其他既有功能都還是用
  單一分類的判斷方式，這次調整範圍限縮在「文章草稿」這個功能，
  不會影響其他既有功能的行為。

三、測試方式

PHP 單元測試：`suggest_categories()` 對一段同時touch到「自主更新」
與「都市更新」關鍵字的問答，正確依分數由高到低回傳多個分類（並
維持既有 `suggest_category()` 單一分類版本行為不變）；對一段刻意
涵蓋多達 12 個分類關鍵字的文字，確認 `$max` 上限（5，以及調整為 3
時）都正確截斷，不會回傳超過上限的分類數。PHP 整合測試：
`UR_AI_FAQ_Article_Service::create_from_faq()` 正確把多個建議分類
全部轉換成 term_id 並指定給文章（不只是單一分類）；重複產生第二篇
文章時，正確沿用既有的多個分類 term、不會重複建立；`UR_AI_FAQ_Ajax`
的 AJAX 回應正確以陣列回傳多個分類，成功訊息正確列出全部分類名稱。
全部情境通過，`php -l` 確認語法正確。

四、是否建議上正式站

建議更新。這是「產生文章草稿」既有功能的分類建議強化，不影響其他
既有頁面／功能的分類判斷方式；唯一的行為變化是新產生的文章草稿，
分類數量可能從固定 1 個變成最多 5 個。
...
使用者請我盤點目前功能、找出「不用大動核心，但可以延伸利用既有
功能」的候選項目後，選定其中 3 項（依原始編號）一起做：（3）知識
大考驗出題來源加入「文章」、（4）FAQ 詳情下方「相關 FAQ」推薦、
（5）知識大考驗答錯題目時優先連結對應文章而非 FAQ。三項功能彼此
關聯緊密（3 與 5 都圍繞「FAQ ↔ 文章」這條既有的 `_ur_ai_source_faq_id`
關聯），因此合併在同一版一起處理。
...
1. `UR_AI_Quiz_Draft_Service` 新增 `generate_from_article($post_id)`／
   `generate_batch_from_articles($post_ids)`：只接受「由本外掛『產生
   文章草稿』建立、且已發布」的文章（透過 `_ur_ai_source_faq_id`
   meta 判斷，且來源 FAQ 仍存在），不接受任意 WordPress 文章，維持
   出題內容一律有可回溯、已審核過的來源。AI 出題改吃文章的標題／
   完整內文（通常比原始 FAQ 回答更豐富），但產生的題目
   `source_faq_id` 仍指向原始 FAQ——這樣既有的答錯複習連結邏輯完全
   不用修改，自動就能沿用。
2. 題庫來源新增 `ai_article`（顯示為「AI 依文章產生」），與既有的
   `manual`／`ai_faq` 並列；`UR_AI_Quiz_Repository` 的來源白名單同步
   更新。
3. 「知識大考驗」後台頁新增「AI 依文章出題」卡片，比照既有「AI 依
   FAQ 出題」的勾選＋批次產生 UI 模式，列出已發布的候選文章。

**（5）答錯複習優先連結文章**

4. `UR_AI_FAQ_Article_Service` 新增 `find_published_article_url
   ($faq_id)`：反查某則 FAQ 是否有一篇「已發布」的文章（只認
   `publish` 狀態，草稿一律不算——與外掛既有「AI 內容一律需人工
   審核」原則一致）。
5. `UR_AI_Quiz_Service::submit_attempt()` 的答錯複習項目，若來源 FAQ
   有已發布的文章，新增 `article_url` 欄位；前台 `quiz.js` 改為優先
   渲染一個連結到該文章的可點擊連結，沒有文章時完全維持原本純文字
   FAQ 提示的行為，不影響任何既有情境。

**（4）FAQ 詳情下方「相關 FAQ」推薦**

6. `UR_AI_FAQ_Service` 新增 `find_related($faq_id, $category, $limit)`：
   依分類查出其他啟用中的 FAQ（排除自己），依命中次數由高到低排序。
7. `UR_AI_Answer_Service::try_faq_answer()`（FAQ 命中時的回答邏輯）
   新增 `related_faqs`，與既有的「相關頁面推薦」（`related_pages`）
   並列附加在回答結果中；`UR_AI_Ajax_Module::format_response()` 新增
   對應的 `format_related_faqs()` 白名單過濾，供前台使用。
8. `public.js` 新增「你也許還想知道」區塊（`renderRelatedFaqs()`），
   點擊任一則相關 FAQ 會直接把該問題重新送出既有的 `ur_ai_ask` 問答
   流程（沿用現成的送出／渲染邏輯，不需要新增 AJAX 端點或單一 FAQ
   詳情頁）。

二、設計原則

- 三項功能都刻意選擇「延伸既有資料與流程」而非新建子系統：出題
  來源沿用既有 `source_faq_id` 欄位（沒有新增資料表欄位）；答錯
  連結沿用既有的 `_ur_ai_source_faq_id` post meta（v1.27.0 就已經
  存在，只是先前只寫入、從未反查）；相關 FAQ 推薦沿用既有的
  `category`／`hit_count` 欄位與「相關頁面推薦」已經驗證過的呈現
  模式（並列而非取代）。
- 相關 FAQ 點擊後「重新送出既有問答流程」而不是打造新的單一 FAQ
  詳情頁／永久連結——目前的 FAQ 知識庫瀏覽頁本來就是條列式排版、
  沒有單一項目的詳情頁面，硬做一個獨立詳情頁反而是「大動」，改為
  重用現成的問答流程是更小的改動。
- 「AI 依文章出題」刻意只接受本外掛自己建立且已發布的文章，不接受
  任意文章，避免出題內容失去「已經人工審核過」的品質保證，也讓
  `source_faq_id` 欄位可以在毫無修改的情況下沿用。

三、測試方式

PHP 整合測試（三個獨立驗證腳本）：
1. 「AI 依文章出題」：驗證合法情境（已發布＋有效來源 FAQ）正確
   呼叫 AI（帶入文章標題／內文而非原始 FAQ 問答）、正確寫入
   `source_faq_id`／`source=ai_article`；文章不存在、非發布狀態、
   沒有來源 FAQ meta、來源 FAQ 已刪除等情境都正確擋下並給出明確
   錯誤訊息；批次處理正確統計成功／失敗筆數；`get_sources()` 正確
   註冊新來源標籤。
2. 答錯複習連結（使用真實 SQLite 資料庫＋真實 `UR_AI_Quiz_Service`）：
   延伸既有的問答複習驗證腳本，新增「來源 FAQ 有已發布文章」與
   「來源 FAQ 沒有已發布文章」兩種情境，確認 `article_url` 只在
   前者出現；重新執行原本（未擴充）的驗證腳本，確認在沒有載入
   `UR_AI_FAQ_Article_Service` 的情況下行為完全不變，沒有引入回歸。
3. 相關 FAQ 推薦（使用真實 SQLite 資料庫＋真實 `UR_AI_FAQ_Service`／
   `UR_AI_Answer_Service`）：驗證排除自己、只取同分類啟用中 FAQ、
   依命中次數排序、正確套用筆數上限、空分類不誤觸全表查詢；驗證
   FAQ 命中回答會正確附加 `related_faqs`；驗證 AJAX 格式化層正確
   過濾掉格式不正確的項目。
全部情境（共 27 項檢查點）通過，`php -l`／`node --check` 確認所有
新增／修改的 PHP、JS 檔案語法正確。

四、是否建議上正式站

建議更新。三項功能都是新增或附加行為，不修改任何既有欄位／既有
呼叫路徑的預設輸出：沒有已發布文章的網站，答錯複習連結行為與
升級前完全相同；FAQ 分類內沒有其他啟用中問答的網站，「你也許還
想知道」區塊不會出現，不影響現有畫面呈現。
...
使用者回報「知識庫管理中的『產生文章草稿』功能，目前好像可以重複
生成，是否不太好」。實際檢查程式碼確認 `UR_AI_FAQ_Article_Service::
create_from_faq()` 完全沒有防止重複產生的檢查，同一則 FAQ 被連續
按下「產生文章草稿」時，每一次都會真的呼叫一次 AI、多建一篇內容
幾乎相同的新草稿——手滑連點會浪費 AI 呼叫費用，也會在文章列表堆出
一堆重複草稿。這一版補上這個檢查。

一、主要新增

1. `UR_AI_FAQ_Article_Service` 新增 `find_pending_draft_article_id
   ($faq_id)`：反查某則 FAQ 是否已有一篇「尚未發布」的文章草稿
   （狀態為 `draft`／`pending`／`future`，不含已丟進回收桶的
   `trash`）。與既有的 `find_published_article_url()` 共用同一個
   新增的私有輔助方法 `find_article_ids_by_faq_id($faq_id,
   $post_statuses)`（依來源 FAQ ID ＋指定文章狀態查詢，兩者只差
   在傳入的狀態陣列不同）。
2. `create_from_faq($faq_id)` 在確認 FAQ 存在後、實際呼叫 AI 之前，
   新增檢查：若該 FAQ 已有尚未發布的文章草稿，直接擋下並回傳明確
   錯誤訊息（帶出既有草稿的文章 ID），提示先審核、發布或刪除現有
   草稿，再重新產生新的文章草稿。

二、設計原則

- 只擋「尚未發布」的草稿，**已發布的文章不算在內**——那代表先前
  那篇已經審核通過確認可以用，管理者之後仍可能想針對同一則 FAQ
  再另外產生一篇新文章（例如 FAQ 內容後來更新過、想換個角度重
  寫），這是合理需求，不應該被永久擋住。
- 已經丟進回收桶（`trash`）的文章也不算在內——那代表管理者已經
  做出「這篇不要」的決定，不應該因為曾經產生過一篇被丟棄的草稿，
  就永久鎖住這則 FAQ 無法再次嘗試產生。
- 沿用既有的 `_ur_ai_source_faq_id` post meta（v1.27.0 起就存在）
  與既有的 `WP_Query` 反查模式（`find_published_article_url()` 在
  v1.33.0 已經建立），沒有新增任何資料表欄位，只是把同一個反查
  邏輯依狀態參數化後重複利用。

三、測試方式

延伸既有的 PHP 整合測試腳本，新增「重複產生防護」情境：已有
`draft`／`pending`／`future` 狀態文章草稿時，`create_from_faq()`
正確擋下並在錯誤訊息中帶出既有草稿的文章 ID，且不會呼叫
`wp_insert_post()`（不浪費 AI 呼叫、不建立多餘文章）；已有
`publish`（已發布）或 `trash`（已丟棄）狀態文章時，不受影響、能
正常產生新的文章草稿。全部情境（原有 22 項＋新增 5 項，共 27 項
檢查點）通過，`php -l` 確認修改的檔案語法正確；重新執行既有的
「知識大考驗答錯複習優先連結文章」驗證腳本，確認
`find_published_article_url()` 改為透過共用輔助方法實作後行為
完全不變，沒有引入回歸。

四、是否建議上正式站

建議更新。這是純粹補強既有功能的防呆檢查，不修改任何既有的成功
路徑輸出格式，也不影響已發布文章或已丟棄草稿的處理方式。
...
上一版（v1.34.0）補上「產生文章草稿」的重複產生防護後，使用者接著
詢問：這個功能能否控制最低的品質文字數量，例如 1500、2000 字？
實際檢查發現最低字數門檻（300 字）與 AI 被要求瞄準的目標字數
（500～900 字）都是寫死在程式碼裡的常數，網站經營者完全無法自行
調整。這一版把這個門檻開放到後台「功能設定」頁，且不只是「調高
門檻」而已——調高後，AI 產生文章時也會同步被要求嘗試寫出對應長度
的內容，避免變成「門檻調高，但 AI 產出還是老樣子，結果大部分都被
擋下」。

一、主要新增

1. `UR_AI_Settings` 新增 `article_min_length` 設定（預設 300，可調整
   範圍 100～3000）與對應的 `get_article_min_length()`，比照既有
   `admin_chat_min_draft_answer_length` 的做法（開放後台調整、正規化
   夾在合理範圍內）。
2. `UR_AI_OpenAI_Client::generate_article_from_faq()` 呼叫 AI 前，
   依這個門檻動態計算兩件事：
   - 系統提示詞要求的目標字數區間：下限維持不低於 500 字、上限為
     下限再加 400 字的緩衝（新增私有方法 `article_length_target()`
     統一計算，避免提示詞文字與 `max_tokens` 各算各的、兜不起來）。
   - 傳給 OpenAI 的 `max_tokens`：依目標字數上限估算需要的 token
     額度（目標上限 × 2 ＋ 300，並限制在 2200～8000 之間），避免
     門檻調高後，AI 還沒寫到要求的長度就先被 `max_tokens` 截斷。
   門檻維持預設值 300 時，計算結果與原本寫死的「500～900 字、
   `max_tokens=2200`」完全相同，不影響任何既有網站的既有行為。
3. `UR_AI_FAQ_Article_Service::create_from_faq()` 建立文章前的品質
   檢查，改為讀取 `UR_AI_Settings::get_article_min_length()`，不再
   使用寫死的 `MIN_ARTICLE_LENGTH` 常數（常數保留作為 `UR_AI_Settings`
   尚未載入時的退回值，但正常情況下一律以設定值為準）。
4. 後台「功能設定」頁「AI 輔助內容品質控管」卡片，新增「產生文章
   草稿最低字數」欄位，與既有「AI 對話草稿最低回答字數」欄位並列。

二、設計原則

- 刻意不是「只調高擋下用的門檻」，而是把門檻同步套用到 AI 被要求
  瞄準的目標字數與 `max_tokens`——單純調高門檻卻不調整 AI 產出，
  只會讓調高後幾乎所有產生結果都被擋下，功能形同壞掉；讓 AI 真的
  嘗試寫出對應長度，門檻才有意義。
- 目標字數下限維持「不低於 500 字」：即使把門檻調得比 500 低，也
  不會反過來縮小 AI 原本就被要求寫出的份量，維持既有文章品質水準
  的下限保證。
- 沿用既有 `admin_chat_min_draft_answer_length` 的設計模式（開放
  後台調整、`get()`／`sanitize()` 正規化夾在合理範圍內），保持
  設定頁與程式碼風格一致。

三、測試方式

延伸既有的 PHP 整合測試腳本，新增「可調整文章最低字數」情境：
維持預設值 300 時，系統提示詞與 `max_tokens` 與原本完全相同（零
行為變化）；調到 1500 時，系統提示詞正確要求「1500～1900 字」、
`max_tokens` 正確提高到 4100；`create_from_faq()` 用同一篇原本能
通過 300 字門檻（303 字）的文章測試，門檻調到 1500 後正確被擋下
（且錯誤訊息帶出 1500），調回 300 後同一篇文章又能正常通過，確認
設定值真正生效且沒有殘留副作用。全部情境（原有 27 項＋新增 4 項，
共 31 項檢查點）通過，`php -l` 確認修改的檔案語法正確；重新執行
既有的「知識大考驗 AI 依文章出題」驗證腳本，確認沒有引入回歸。

四、是否建議上正式站

建議更新。預設值與計算結果都與升級前完全相同，沒有調整過這個
設定的網站行為不受影響；有調整需求的網站，可自行到「功能設定」
頁依需要調整為 1500、2000 或其他合適的數值。
...
使用者詢問「上次有提到稅賦計算、契稅、土增稅的功能（都更權利變換有
減徵的節稅），是否可行」。經查證財政部稅務入口網公開的土地增值稅
試算公式與契稅條例稅率表，並與都市更新條例第67條、都市危險及老舊
建築物加速重建條例（危老條例）第8條的法條原文交叉核對後確認可行，
使用者指示「排除危老條例，並說明減徵僅針對都更」、「先做第一層
（一般稅額），如果有把握再一起完整兩層」。這一版新增獨立的「稅賦
試算」模組，一次完成一般稅額計算（第一層）與都市更新條例減免情境
（第二層）。

一、主要新增

1. 新增 `UR_AI_Tax_Calculator_Service`（純 PHP，無 WordPress 依賴）：
   - `calculate_land_value_increment_tax()`：依土地稅法第33條公式計算
     土地增值稅一般稅額——自用住宅用地（稅率10%，都市土地上限300
     平方公尺／非都市土地700平方公尺，超過部分自動分攤按一般稅率
     計算）；一般用地依漲價倍數分3級累進課徵，並依持有年限（20／
     30／40年級距）套用對應的長期持有減徵率，數字與財政部稅務入口
     網公告的「一般用地稅率速算表」逐項核對一致。
   - `calculate_deed_tax()`：依契稅條例第3條稅率表計算契稅（買賣6%、
     典權4%、交換2%、贈與6%、分割2%、占有6%）。
   - `resolve_urban_renewal_reduction()`：都市更新條例第67條第1項各
     款減免情境（權利變換取得首次移轉、現金補償、協議合建、以房地
     抵充負擔、未達最小分配面積改領現金），逐項標明法條依據；**明確
     列出「危老重建」情境並標示不適用**，附上「危老條例第8條僅有
     房屋稅／地價稅減半，並無土地增值稅或契稅減免規定」的說明，
     避免使用者誤以為都更的40%減免也適用於危老重建。
   - 現金補償（第3款）與協議合建（第8款）減免訂有實施期限（原5年，
     行政院112年12月29日院臺建字第1121043926號令展延至118年即西元
     2029年1月31日止），服務層會依目前日期自動判斷是否仍在適用期間
     內，超過期限後自動改按一般稅額計算並附上到期說明，避免展延屆
     滿後仍誤用已失效的優惠。
2. 新增 `UR_AI_Tax_Calculator_Settings`／`UR_AI_Tax_Calculator_Ajax`／
   `UR_AI_Tax_Calculator_Module`：前台 shortcode `[ur_ai_tax_calculator]`
   （土地增值稅／契稅兩個分頁，試算結果附完整公式拆解過程，比照
   既有「都更分回試算」的公式透明呈現原則）；後台「稅賦試算」設定
   頁僅開放「是否啟用」與「免責聲明文字」——稅率、級距、減免比率
   為法定數字，刻意不開放調整，避免管理者誤改成與法規不符的數字。
3. 依 `UR_AI_Industry_Profiles` 判斷預設啟用狀態：都更危老、自主
   更新兩個產業別預設啟用；地政士產業別也預設啟用（一般土地增值稅
   ／契稅試算是地政士業務的核心需求，`docs/industry-expansion-
   architecture.md` 先前就已將「稅費試算」列為地政士產業別的未來
   擴充方向）。

二、設計原則

- 都更減免情境清單刻意包含「危老重建」並標示不適用，而不是直接不
  提供這個選項——避免使用者誤以為工具漏掉這個情境，或誤套用都更
  的40%減免到危老重建案件上。
- 現金補償／協議合建減免的落日期限直接寫在程式碼常數
  （`URBAN_RENEWAL_SUNSET_EXPIRES_AT`）並附註目前的展延依據，方便
  未來行政院再次展延或屆滿失效時，只需要更新這一個常數。
- 稅率、級距、都更減免比率一律不開放後台調整，只有「是否啟用」與
  「免責聲明文字」可調——這些是法定數字，開放調整反而會讓管理者
  不小心改出與法規不符的試算結果。
- 試算結果一律附上完整拆解過程（土地漲價總數額、漲價倍數、適用
  級距、套用的減免與計算方式），不是只給一個總金額，方便使用者
  核對每一步計算是否符合自己的實際情況——延續「都更分回試算」的
  公式透明原則。

三、測試方式

PHP 整合測試（兩個獨立驗證腳本，共 35 項檢查點）：
1. `UR_AI_Tax_Calculator_Service`（26 項）：土地增值稅第1／2級稅率
   計算與手算結果核對一致；自用住宅用地超過都市土地300平方公尺／
   非都市土地700平方公尺上限時，正確分攤為自用與一般兩部分分別
   計稅；持分分子分母正確換算有效面積；輸入驗證（面積或現值為0）
   正確擋下；契稅6種移轉原因稅率正確且未知類型有安全退回值；都更
   減免情境正確套用40%減徵或免徵；「現金補償」情境正確排除契稅
   （無對應契稅事件）；「危老重建」情境在土地增值稅與契稅兩種稅目
   都正確標示不適用並附法條說明；落日期限判斷正確（到期前仍適用、
   到期後自動改為不適用並附到期說明，且「權利變換首次移轉」等無
   落日期限的情境不受影響）。
2. `UR_AI_Tax_Calculator_Ajax`＋`UR_AI_Tax_Calculator_Settings`（9
   項，使用真實 production 類別＋模擬 WordPress 函式）：設定的
   啟用狀態正確反映所屬產業別的模組開關；AJAX 端點正確回傳土地
   增值稅／契稅試算結果；危老排除說明正確透過 AJAX 層完整傳遞到
   前台；無效輸入、功能已停用、nonce 驗證失敗、超過頻率上限皆正確
   以乾淨錯誤回應處理，沒有未攔截的例外。
3. 重新執行既有的產業別／品牌名稱／推廣連結相關驗證腳本，確認在
   `modules` 設定陣列新增 `tax_calculator` 鍵之後沒有引入回歸（兩個
   既有失敗案例經比對為修改前就存在的舊版驗證腳本問題，與本次改動
   無關）。

全部情境通過，`php -l`／`node --check` 確認所有新增檔案語法正確。

四、是否建議上正式站

建議先在測試環境實際跑過幾組已知答案的案例（例如財政部稅務入口網
的試算範例），確認畫面呈現與拆解說明清楚易懂後，再對外開放。所有
數字皆為法定公式概算，非正式稅捐核定金額，這一點已在免責聲明與
畫面文案中明確提醒。
...
使用者指出「稅賦要增加友善列印功能」——v1.36.0 新增的稅賦試算沒有
比照既有「都更分回試算」提供友善列印，試算結果只能整頁截圖或直接
印整個頁面（含分頁按鈕、輸入表單），不適合列印留存或交給客戶參考。

一、主要新增

1. 前台試算結果新增「🖨 友善列印」按鈕（`tax-calculator-view.php`
   結果區新增「日期＋列印」列，比照既有「都更分回試算」的呈現）。
2. `tax-calculator.js` 新增 `printResult()`：做法完全比照
   `calculator.js` 既有的友善列印——建立一個隱藏 iframe，把結果區
   複製一份乾淨副本（移除列印按鈕本身），套用專用的列印樣式表後寫入
   iframe 並觸發 `print()`，印出後自動移除 iframe。只印試算結果
   本身（含公式拆解、都更減免說明、免責聲明、網站品牌頁尾），不會
   把分頁按鈕、輸入表單或另一個分頁的隱藏欄位一起印出來。
3. `tax-calculator.css` 新增對應樣式：列印頁尾（`__print-footer`）
   平常在畫面上隱藏，只在列印輸出中顯示（沿用既有計算機的做法）。

二、設計原則

- 刻意重用既有「都更分回試算」已經驗證過的列印做法（隱藏 iframe＋
  獨立列印樣式表），而不是另外設計一套新機制，維持兩個試算器的
  使用體驗一致。

三、測試方式

以 Playwright 驅動真實瀏覽器（Chromium）操作整個流程：渲染真實的
`tax-calculator-view.php` 樣板＋載入真實的 `tax-calculator.css`／
`tax-calculator.js`（僅將 AJAX 回應以假資料攔截，其餘皆為正式程式
碼），實際點擊分頁、送出試算、確認結果正確渲染後，再點擊「友善
列印」按鈕，驗證：動態建立的 iframe 內容正確排除列印按鈕本身、
正確包含試算結果數字、公式拆解、免責聲明與網站品牌頁尾；列印觸發
後 iframe 會在短暫延遲後自動從畫面移除；全程無 JavaScript
console 錯誤。

四、是否建議上正式站

建議更新。純前端新增功能，不影響任何既有計算邏輯或既有頁面元素。
...
使用者回報「發現無法新增頁面，都在轉圈圈、空白狀況」，停用本外掛後
恢復正常。取得使用者的 `debug.log` 後確認：這跟這次新增的稅賦試算
或友善列印完全無關（這兩個功能只在使用者實際使用 shortcode／設定頁
時才會執行，不會影響 WordPress 內建的「新增頁面」）。真正原因是
`UR_AI_Schema_Manager::maybe_upgrade()` 掛在 `plugins_loaded`，等於
「每一次」網站請求（前台與後台皆然，包含與本外掛完全無關的「新增
頁面」）都會被呼叫一次；而使用者的主機環境（WordPress 核心版本與
PHP 版本組合）觸發了 WordPress 核心 `dbDelta()`（`wp-admin/includes/
upgrade.php`）本身解析資料表 KEY／INDEX 子句失敗的相容性問題（非本
外掛 SQL 語法錯誤——本外掛沿用標準 `PRIMARY KEY  (id)` / `KEY name
(column)` 寫法），導致資料表升級一直無法判定為「已完成」，於是
`maybe_upgrade()` 沒有節流機制時，就會在「每一個」頁面請求都重新
執行一次完整的 `dbDelta()` 升級流程（對 8 張資料表逐一比對），這個
過程本身有開銷，多次疊加後就會讓整個後台明顯變慢、甚至看起來像是
卡住轉圈圈。

一、主要修正

1. `UR_AI_Schema_Manager::maybe_upgrade()` 新增重試節流：資料庫版本
   比對不相符時，先檢查一個 transient 鎖（`RETRY_LOCK_TRANSIENT`，
   有效期 `RETRY_THROTTLE`＝1 小時），若鎖還在就直接略過，不再呼叫
   `install()`／`dbDelta()`；沒有鎖才會真正嘗試升級一次，並立刻上鎖。
   這樣即使升級持續失敗（例如主機環境的 dbDelta 相容性問題尚未排除），
   最多每小時才會重試一次，不會再每個頁面請求都重新執行一次完整的
   升級流程。

二、設計原則

- 這是「防禦性」修正：不論升級失敗的根本原因是什麼（主機 dbDelta
  相容性問題、資料庫帳號權限不足、暫時性連線問題……），只要
  `maybe_upgrade()` 沒有節流機制，任何一種持續性失敗都會拖慢整個
  網站的每一個頁面請求。加上節流後，即使升級一直卡住，最多只會每
  小時付出一次額外開銷，而不是每個頁面都要付出。
- 一旦升級真正成功（`all_tables_exist()` 成立、`DB_VERSION_OPTION`
  正確寫入），`maybe_upgrade()` 會在檢查資料庫版本這一步就直接
  結束，連 transient 鎖都不會去讀取，效能與升級前完全相同，不影響
  正常運作的網站。
- 建議使用者仍應將 WordPress 核心更新到最新版本，這是解決
  `dbDelta()` 這類 PHP 版本相容性警告最根本的做法；這次的節流修正
  是讓「萬一升級持續卡住」這件事不會拖累整個網站，而不是取代更新
  WordPress 核心。

三、測試方式

PHP 整合測試（使用真實的 `UR_AI_Schema_Manager` 類別，不需要真實
`$wpdb`／`dbDelta`——刻意不定義任何 schema class，讓
`get_table_statuses()`／`all_tables_exist()` 自然而然一直回傳
「尚未完成」，精準重現使用者回報的「升級持續失敗」情境）：確認
第一次呼叫會真正嘗試升級並上鎖；模擬同一小時內再發生 10 次頁面
請求，確認鎖持續有效、不會重複觸發升級；確認節流時間一到，下一次
呼叫會再次真正嘗試升級；確認升級一旦成功（`DB_VERSION_OPTION`
寫入），後續呼叫會在最前面就直接結束，完全不會碰觸節流鎖。全部
6 項檢查點通過，`php -l` 確認修改的檔案語法正確。

四、是否建議上正式站

建議更新，且優先順序高於其他功能性更新——這是效能／穩定性修正，
對「資料庫升級曾經卡住過」的網站特別重要；對升級一直正常的網站則
完全零影響。
...
延續 v1.36.2 對「新增頁面轉圈圈」問題的修正——使用者確認更新後恢復
正常，但接著問「感覺速度變慢了，是否是外掛的影響」，並表示「外掛
如果影響整體網站效能，不會受到歡迎」。這個疑慮完全合理：v1.36.2
只是把「持續重試升級」的頻率從每個頁面降到每小時一次，並沒有解決
「dbDelta 可能沒有正確建立索引」這個根本問題本身；如果真的有索引
缺漏，查詢會隨著資料筆數增加而越來越慢，且不會有任何明確錯誤訊息
提醒。這一版新增一個不需要任何技術背景就能操作的診斷與修復工具，
讓使用者自己就能確認、也能自己排除這個疑慮。

一、主要新增

1. `UR_AI_Schema_Manager` 新增 `get_index_health_report()`：對 8 張
   資料表逐一比對「應該要有的索引」（直接解析各 schema 自己
   `get_sql()` 裡的 `KEY`／`UNIQUE KEY` 子句，包含複合欄位索引與
   唯一索引）與「資料庫裡實際存在的索引」（`SHOW INDEX FROM`），
   找出缺漏，不需要用猜的。
2. `UR_AI_Schema_Manager` 新增 `repair_missing_indexes()`：針對缺少
   的索引，直接下單純的 `ALTER TABLE ... ADD INDEX`／`ADD UNIQUE
   INDEX`，**刻意不重新呼叫 dbDelta()**——dbDelta 正是先前索引沒有
   正確建立的來源，繞開它、改用最簡單直接的語法，才能真正解決問題
   而不是重蹈覆轍。
3. 後台「總覽」頁新增「資料庫索引健康檢查」卡片：一個「檢查資料庫
   索引健康」按鈕，結果用白話文顯示（✅ 全部正常 / ⚠️ 哪幾張表缺少
   幾個索引），若有缺漏會出現「立即修復缺少的索引」按鈕，一鍵完成
   修復並顯示修復結果（成功幾個、失敗幾個及失敗原因）。全程不需要
   任何 SQL 或資料庫知識。

二、設計原則

- 檢查動作刻意只在管理者主動點擊「檢查」時才查詢資料庫（GET 帶
  nonce 驗證），不會在每次造訪總覽頁時都自動執行——避免這個「確認
  外掛沒有拖慢網站」的診斷工具，自己又變成拖慢頁面的額外負擔，這
  正是這次要解決的核心疑慮，不能自相矛盾。
- 修復動作要求「功能設定」等級的權限（比單純檢視總覽頁更嚴格）並
  驗證 nonce，且只會「新增」缺少的索引，不會刪除或修改任何既有
  資料表結構或資料內容，可安全重複執行。
- 索引名稱、欄位、是否唯一索引，都是直接從各 schema 檔案自己的
  `get_sql()` 動態解析取得，不是另外維護一份寫死的期望清單——未來
  若有 schema 新增欄位索引，這個健康檢查會自動涵蓋，不需要另外
  同步修改。

三、測試方式

PHP 整合測試（兩個獨立驗證腳本，共 17 項檢查點）：
1. `get_index_health_report()`／`repair_missing_indexes()`（12 項，
   使用模擬的 `$wpdb` 記錄每一筆下達的 SQL，並載入全部 8 個真實
   schema class 呼叫真實的 `get_schema_classes()`，只針對其中一張
   表模擬「dbDelta 只建立了 PRIMARY KEY，其餘索引都沒建立成功」的
   故障情境）：確認從真實 schema SQL 正確解析出唯一索引、複合欄位
   索引；確認 8 張資料表全部正常時回報全部健康；確認只有刻意弄壞
   的那一張表被回報不健康、缺少的索引名稱完全正確，其餘 7 張表
   不受影響；確認資料表不存在時能乾淨地回報、不會嘗試對不存在的
   資料表下 `SHOW INDEX` 而出錯；確認修復動作下達的是單純的
   `ALTER TABLE ADD INDEX`（從未呼叫 dbDelta）；確認修復報告正確
   列出成功與失敗的索引，其中一個索引刻意模擬修復失敗時，其餘索引
   仍然正常修復完成，失敗原因清楚呈現、不會被靜默吞掉。
2. 後台總覽頁渲染測試（5 項，直接渲染真實的 `dashboard-page.php`）：
   一般造訪頁面時，健康檢查卡片會出現，但**不會**自動執行檢查（沒有
   額外資料庫查詢負擔）；帶正確 nonce 觸發檢查時，正確顯示警示與
   修復按鈕；帶錯誤或缺少 nonce 時，即使網址參數存在也不會執行
   檢查（安全性驗證確實生效）。

全部情境通過，`php -l` 確認所有修改檔案語法正確。

四、是否建議上正式站

建議更新。這是純粹新增的診斷工具，讀取動作預設不主動執行、修復
動作只會新增缺少的索引，不影響任何既有功能或資料，對「懷疑外掛
拖慢網站效能」的使用者特別有價值——能自己確認、自己排除疑慮，
不需要等待或依賴開發者才能查證。
...
使用者回報 v1.36.3 之後，「新增頁面」不再持續轉圈圈，但**第一次**
打開時仍會顯示空白，重新整理後才能正常顯示。取得使用者當下重現
時間點的 `debug.log` 後確認：紀錄裡只有大量重複的 WordPress 6.7+
`_load_textdomain_just_in_time` Notice（「翻譯字串載入時機過早」），
沒有其他錯誤。這是這個外掛過去就存在、先前只被當成單純的次要提示
（見 v1.34.0 前後的討論），但這次配合「區塊編輯器（Gutenberg）新增
頁面時會發出大量內部 REST API 請求」的行為交叉比對後，確認這正是
「第一次空白、重新整理才正常」這個症狀的根本原因：主機環境把 PHP
Notice 直接輸出到頁面時，若這段文字剛好夾雜在區塊編輯器某個內部
REST API 請求「應該是純 JSON」的回應內容裡，前端 `JSON.parse()`
會失敗，導致編輯器該次初始化失敗、畫面空白；重新整理後若剛好那次
沒有再次觸發同樣的時機問題，就能正常顯示——這也解釋了為什麼上一版
的「新增頁面轉圈圈」修好之後，仍然殘留這個看似隨機、只發生一次的
空白狀況。

一、主要修正

1. `UR_AI_Bootstrap::run()`：`boot_modules()`（負責啟動全部模組、
   內部會呼叫大量翻譯字串，例如 `UR_AI_Industry_Profiles::profiles()`
   建立各產業別預設值時用到的 `__()`）原本與外掛主要啟動流程同步
   掛在 `plugins_loaded`（早於 WordPress 的 `init`），改為掛在
   `init`（優先權 0，盡量搶在其他外掛的 init 掛鉤之前執行，確保
   後續各模組自己註冊的 `admin_menu`／`wp_enqueue_scripts`／
   shortcode 等掛鉤仍有充足時間被 WordPress 正確觸發），完全比照
   WordPress 官方 Notice 訊息本身建議的「翻譯字串應該在 init 或之後
   才載入」。
2. `UR_AI_Updater::init()`（外掛更新檢查）維持原本掛在 `plugins_loaded`
   不變——這是 Plugin Update Checker 官方建議的時機，且這段程式碼
   本身不涉及任何翻譯字串呼叫，不受這次時機問題影響，不需要跟著
   調整。

二、設計原則

- 只調整「啟動全部模組」這一段的觸發時機，不更動任何模組內部的
  邏輯或彼此之間的執行順序——`register_default_modules()` 與
  `boot_modules()` 這兩個呼叫仍然緊接著依序執行，只是整體從
  `plugins_loaded` 延後到 `init` 才開始，對外部行為（shortcode、
  AJAX、後台選單何時可用）沒有任何影響，因為這些掛鉤原本就是設計
  給比 `init` 更晚觸發的事件（`admin_menu`、`wp_enqueue_scripts`）
  使用。
- 沒有逐一去找出、修改散落在各個檔案裡呼叫翻譯字串的所在位置——
  那樣的做法既沒有效率、也容易掛一漏萬；改成從源頭調整「整個模組
  啟動流程」的觸發時機，一次性、徹底地解決問題，且未來新增的模組
  只要延續現有的 register()／boot() 架構，也會自動受惠、不會重蹈
  覆轍。

三、測試方式

PHP 整合測試（3 項檢查點，模擬真實 WordPress 請求生命週期：先觸發
`plugins_loaded`、再觸發 `init`，並用一個會呼叫翻譯字串的假模組
標記每次呼叫發生在哪個階段）：確認 `plugins_loaded` 階段完全沒有
任何翻譯字串呼叫發生（本次修正的核心）；確認模組的
`register()`／`boot()`（連同其中的翻譯字串呼叫）在 `init` 觸發時
正確執行；確認整個延後機制確實有讓模組正常啟動，不是被誤刪或遺漏
沒有執行。全部情境通過，`php -l` 確認修改的檔案語法正確。

四、是否建議上正式站

建議更新。這是修正外掛啟動時機、避免觸發 WordPress 核心已知警告
訊息的根本性修正，不改變任何功能行為，預期能解決「新增頁面第一次
顯示空白、重新整理才正常」的殘留症狀。
...
新增「都市更新共同負擔提列估算」功能（第一階段），依「新北市」政府
公告之都市更新事業及權利變換計畫作業須知——提列總表（附表一）與各
分項說明（附表二～附表八）之公開公式與費率表，讓使用者輸入基地、
樓層、面積、戶數、權屬等條件後，概算共同負擔中「有明確公式」的項目。
這是繼稅賦試算（v1.36.0）之後，第二個把政府公開法定公式做成「輸入
條件、算出金額」工具的功能，同樣採「分階段」推進：本版先完成有明確
公式、僅需幾何／戶數條件即可計算的第一階段，稅捐E（含營業稅）、
銷售管理費F4與共同負擔比率（需主管機關核定之更新後總權利價值）留待
第二階段。

一、主要新增功能

1. 前台 shortcode `[ur_ai_joint_burden]`：都市更新共同負擔提列估算
   （新北市）。分四區輸入——(一)建物與工程條件、(二)更新單元與權屬、
   (三)費率與貸款參數、(四)個案認定項目（選填）——一次算出下列項目
   並附完整逐項公式明細（比照稅賦試算，結果可友善列印）：
   - 工程費用 A：拆除費用（依 8 種構造單價表）、營建費用（依構造別×
     總樓地板面積級距×樓層區間之標準單價表，物價基準日 112 年 4 月，
     可選填加成率與物價指數調整）、外接水電瓦斯管線費用（每戶 7.5 萬）。
   - 權利變換費用 C：都市更新規劃費（P1＋P2＋P3，含面積級距 X 與
     權利人數級距 Y 之累進計算）、不動產估價費（服務費＋選定費）、
     土地鑑界費（每筆 4 千）、鑽探費（每孔 7.5 萬）、地籍整理費
     （每戶 2 萬）。
   - 貸款利息 D：依施工期間（地下層首層 4 月、每多一層 ＋2 月；地上層
     每層 1 月；可勾選逆打工法）＋12 月推算貸款期間，年利率依自有資金
     比率／郵政定存／五大銀行基準利率加權計算。
   - 管理費用 F：行政作業費 F1（土地公告現值總值×2.5%）、信託費 F2
     （選填，一般建商折半）、人事行政管理費 F3（(A＋C＋G)×費率表，
     依產權級別與基地面積）、風險管理費 F5（(A＋C＋D＋F1＋F2＋F3＋
     G＋H)×費率表，依產權級別與總樓地板面積）。
   - 個案認定項目（建築設計費、工程管理費、公共及公益設施、拆遷補償
     ／安置費、都市計畫變更負擔 G、容積移轉費 H 等）做成選填欄位，
     有數字才計入，並一併影響 D、F3、F5 的計算。
2. 後台新增「共同負擔估算」設定頁：啟用開關與免責聲明文字。單價表、
   費率、級距屬新北市公告基準，不開放後台調整（避免誤改成與公告不符
   的數字），比照稅賦試算的設計。

二、設計原則

- 只計算「有明確公式、可驗證」的項目，並在結果頁明確標示：本工具目前
  僅依「新北市」提列基準計算，臺北市及其他縣市之公式與費率不同不適用；
  第一階段尚未計入稅捐E、銷售管理費F4與B項、也未計算共同負擔比率。
  個案認定項目一律標示「個案」並保留選填，不假裝能算出精確值，避免給
  使用者錯誤的精確感。
- 純 PHP 服務層（UR_AI_Joint_Burden_Service），單價表、費率表、級距
  皆為類別常數，所有輸出保留逐項公式與金額拆解，方便使用者核對。
- 完全比照既有稅賦試算模組的架構（Service／Settings／Ajax／Module
  四件式、獨立 option、rate-limit、友善列印），延續一致的維護方式。

三、新增檔案

- includes/modules/joint-burden/class-ur-ai-joint-burden-service.php
- includes/modules/joint-burden/class-ur-ai-joint-burden-settings.php
- includes/modules/joint-burden/class-ur-ai-joint-burden-ajax.php
- includes/modules/joint-burden/class-ur-ai-joint-burden-module.php
- public/views/joint-burden-view.php
- public/assets/css/joint-burden.css
- public/assets/js/joint-burden.js
- admin/pages/joint-burden-settings-page.php

四、修改檔案

- ur-ai-assistant.php（版本號 1.36.4 → 1.37.0）
- includes/core/class-ur-ai-autoloader.php（新增 4 個類別對應）
- includes/core/class-ur-ai-module-manager.php（註冊 joint_burden 模組）
- includes/core/class-ur-ai-industry-profiles.php（都更／自主更新產業別
  預設啟用、地政士預設關閉）
- admin/pages/dashboard-page.php（Shortcode 使用說明新增第 9 組、
  組數 8→9）
- docs/changelog.md、readme.txt

五、資料表變更

無。本功能不使用資料表，僅新增一個獨立 option
（ur_ai_joint_burden_settings）。

六、測試方式

- PHP 服務層單元測試（32 項檢查點）：以官方公式手算對照——拆除單價、
  營建單價表查表（含 <6 層與 rc 無 36+ 級距的退回處理）、物價指數
  調整（>2.5% 才調）、加成率、規劃費 X／Y 累進級距、估價費（含選定費
  25 萬下限與 30% 上限）、F3／F5 費率查表、貸款利息施工期間推算（含
  逆打工法）、以及完整 A→C→D→F 串接與 subtotal 一致性，全部通過。
- PHP 全生命週期整合測試（17 項檢查點）：以 WordPress 函式 stub 驅動
  真實模組類別——construct→register→boot→admin_menu→wp_enqueue_scripts
  →shortcode 渲染→AJAX collect_args→設定頁渲染，確認 shortcode／AJAX
  ／後台選單正確註冊、前台 HTML 正常輸出、百分比輸入正確換算為比率、
  服務計算成功，全部通過。
- `php -l` 確認全部新增與修改的 PHP 檔案語法正確；`node --check` 確認
  前台 JS 語法正確。

七、是否建議上正式站

建議更新。純新增功能，預設在「都市更新／自主更新」產業別啟用、
「地政士」產業別關閉，不影響既有功能；不使用資料表，風險低。
...
「都市更新共同負擔提列估算」完成第二階段，把 v1.37.0 的第一階段
（工程費用A、權利變換費用C、貸款利息D、管理費用F1／F3／F5）補齊為
完整的 A～H 結構，新增稅捐E（印花稅＋營業稅）、銷售管理費F4、容積
獎勵維護費B，並可計算「共同負擔比率」。至此本工具已能依新北市提列
基準，從輸入條件一路算到「共同負擔總額」與「共同負擔比率」。

一、主要新增功能

1. 稅捐 E：
   - 印花稅：承攬契據依印花稅法第 5 條千分之一，以營建費用為稅基概估。
   - 營業稅：依財政部 109 年 9 月 14 日台財稅字第 10900611910 號令釋
     公式計算。此公式為「營業稅 =（更新後總權利價值 − 共同負擔）×
     比例 × 5%」，而其中的「共同負擔」本身又包含營業稅，屬循環定義；
     本版以代數推導出封閉解
     「營業稅 =（V − K）× r×5% ÷（1 + r×5%）」（K 為共同負擔扣除
     營業稅後之金額），一次算出、不需反覆迭代。比例 r 依令釋「擇一
     計算」提供兩種：房屋評定比例法（房評 ÷（土地公告現值＋房評））、
     費用比例法（(K − 公共設施用地負擔) ÷ V）。
2. 銷售管理費 F4：依實施者實際獲配之單元及車位總價值分級累進計算
   （25 億以下 6%、25-50 億 5.5%、50 億以上 5%，分級累加）。
3. 共同負擔比率：填入「更新後總權利價值」後，計算
   共同負擔 ÷ 更新後總權利價值，並在比率低於 40% 時提示自組更新會
   得將 F5 費率提列至 14% 之規定（僅提示，不自動套用，須經審議會同意）。
4. 個案選填新增「容積獎勵後續管理維護費 B」。
5. 前台結果頁改版：標題列依是否已輸入更新後總權利價值，動態顯示
   「共同負擔總額」或「共同負擔（不含營業稅）」，並新增共同負擔比率
   顯示列與「稅捐 E」費用分組；未輸入更新後總權利價值時明確提示
   「顯示金額不含營業稅」。

二、設計原則

- 營業稅循環定義以封閉解一次算出，避免迭代收斂與精度問題；並在
  試算明細中完整說明 K、r、公式，維持逐項透明可核對的一貫做法。
- 未輸入「更新後總權利價值」時，不臆造營業稅與比率，改以明確提示
  告知結果僅到不含營業稅的部分，避免誤導。
- 仍僅適用新北市提列基準，結果頁與設定頁均明確標示臺北市及其他縣市
  不適用。

三、修改檔案

- ur-ai-assistant.php（版本號 1.37.0 → 1.38.0）
- includes/modules/joint-burden/class-ur-ai-joint-burden-service.php
  （新增 calculate_sales_management_fee()、calculate_business_tax()、
  印花稅、共同負擔比率，並將 calculate() 擴充為完整 A～H）
- includes/modules/joint-burden/class-ur-ai-joint-burden-ajax.php
  （新增第二階段輸入參數）
- includes/modules/joint-burden/class-ur-ai-joint-burden-settings.php
  （更新預設免責聲明）
- public/views/joint-burden-view.php、public/assets/js/joint-burden.js、
  public/assets/css/joint-burden.css
- admin/pages/dashboard-page.php（更新第 9 組 shortcode 說明）
- readme.txt
...
- PHP 服務層第二階段單元測試（20 項檢查點）：F4 分級累進三段驗證、
  營業稅兩種比例法之封閉解與「代入後滿足循環定義」的定點驗證、V=0
  的邊界處理、完整 A～H 串接下 total_burden = A+B+C+D+E+F+G+H 與
  共同負擔比率的恆等式、印花稅 = 營建×0.1%、以及未輸入更新後總權利
  價值時營業稅為 0／比率為 null 的行為，全部通過。
- 第一階段既有 32 項單元測試與全生命週期整合測試（擴充為 23 項，
  加入第二階段輸入與 E／F4／比率斷言）全部通過。
- `php -l`／`node --check` 語法檢查通過。

六、是否建議上正式站

建議更新。延續 v1.37.0 的純新增功能，補齊共同負擔完整計算，不影響
既有功能、不使用資料表，風險低。
...
強化「共同負擔提列估算」的參考性質提醒。都市更新共同負擔因個案條件
差異大、且最終須經審議會審查委員認定，工具算出的金額僅能作為事前
概算，不能被誤當成定案數額。本版把這個提醒做得更明顯，避免使用者
過度依賴試算結果。

一、主要調整

1. 前台輸入區上方新增明顯的紅色警語橫幅：本估算僅供事前概算參考，
   實際金額會因個案條件（營建單價調整、各項個案認定費用、審查認定
   範圍等）而有差異，最終一律以「都市更新及爭議處理審議會」審查委員
   審議通過之數額為準。
2. 試算結果區（含友善列印輸出）在總額下方新增同樣的提醒短語，確保
   列印出來的紙本也帶有此警語。
3. 試算明細（notes）與後台免責聲明預設文字，均改為以此提醒開頭。

二、修改檔案

- ur-ai-assistant.php（版本號 1.38.0 → 1.38.1）
- public/views/joint-burden-view.php、public/assets/css/joint-burden.css、
  public/assets/js/joint-burden.js（列印樣式）
- includes/modules/joint-burden/class-ur-ai-joint-burden-service.php
  （試算 notes 首則提醒）
- includes/modules/joint-burden/class-ur-ai-joint-burden-settings.php
  （預設免責聲明）
...
`php -l`／`node --check` 確認修改檔案語法正確；既有服務層與全生命
週期測試不受影響（僅文案與樣式調整，計算邏輯未變）。

五、是否建議上正式站

建議更新。純文案與提醒強化，不影響任何計算邏輯與既有功能。
...
「共同負擔提列估算」的「建築設計費用」從純手動輸入，升級為可依
「建築師酬金標準表」自動估算。新北市提列總表對建築設計費的規定是
「依造價標準表及建築師公會建築師酬金標準表計算」——工具已算出營建
費用（即總工程費），故可接續以酬金費率算出設計費。

一、主要新增功能

1. 建築設計費自動估算：依「全國建築師公會版建築師酬金標準表」
   （全建師會 111 字第 0030 號），以營建費用為總工程費，按建築物
   種別與總工程費級距累進計算：
   - 種別：一般建築／公共及高層建築／特殊建築（都更集合住宅預設
     「公共及高層建築」）。
   - 級距（累進）：≤300萬、300萬-1500萬、1500萬-6000萬、>6000萬，
     各級距下限費率依種別不同（公共及高層為 7/6/5.5/5%），上限一律
     9.0%。
   - 因費率為可協議之區間（下限～9%），系統同時算出「下限累進金額」
     與「上限 9% 金額」，共同負擔小計採下限概算，明細顯示完整區間。
2. 使用者可於「個案認定項目」切換建築設計費計算方式（自動／手動）與
   建築物種別；選手動時沿用原本的金額輸入。

二、設計原則

- 誠實揭露：明確標示此為「全國建築師公會」版本，非新北市建築師公會
  自訂版本，實際新北市個案仍應以新北市建築師公會酬金表為準；且費率
  屬可協議區間，工具採下限概算、非唯一定值。
- 總工程費採用工具內部已算出的營建費用，與新北市提列總表「依造價表
  及酬金表計算」的規定一致。

三、修改檔案

- ur-ai-assistant.php（版本號 1.38.1 → 1.39.0）
- includes/modules/joint-burden/class-ur-ai-joint-burden-service.php
  （新增 calculate_design_fee()，calculate() 支援自動／手動設計費）
- includes/modules/joint-burden/class-ur-ai-joint-burden-ajax.php
  （新增 design_fee_mode／design_fee_category 參數）
- public/views/joint-burden-view.php、public/assets/js/joint-burden.js
- readme.txt
...
- PHP 服務層新增建築設計費測試（10 項檢查點）：三種種別於 1 億總
  工程費的下限累進金額手算對照、上限 9% 金額、不明種別退回預設、
  小案單一級距、以及全鏈 auto／manual 兩種模式下設計費金額與 auto
  標記正確，全部通過。
- 既有第一階段 32 項、第二階段 30 項、全生命週期 23 項測試全部通過；
  `php -l`／`node --check` 通過。

六、是否建議上正式站

建議更新。純新增／強化功能，設計費預設改為自動估算（採保守下限），
可隨時切回手動，不影響其他計算項目。
...
延續 v1.39.0，將「公寓大廈公共基金」也從純手動輸入升級為可自動估算。
新北市提列總表的公共基金項目規定「依公寓大廈管理條例施行細則第5條
規定計算」，該條有明確的累進提列公式，故可比照設計費做成自動計算。

一、主要新增功能

1. 公寓大廈公共基金自動估算：依公寓大廈管理條例施行細則第5條，以
   工程造價累進提列——1千萬元以下 20‰、逾1千萬至1億元部分 15‰、
   逾1億至10億元部分 5‰、逾10億元部分 3‰。條文所稱工程造價指
   「建造執照載明之工程造價」，本工具以已估算之營建費用近似。
2. 使用者可於「個案認定項目」切換公共基金計算方式（自動／手動）；
   選手動時沿用原本的金額輸入。

二、設計原則

- 誠實揭露：明確標示工程造價應以建照載明造價為準，本工具以營建費用
  近似。
- 自動計算之公共基金金額仍正確流入貸款利息 D 公式中的
  「（工程費用A − 公寓大廈管理基金）」扣除項，維持 D 的計算正確性。

三、修改檔案

- ur-ai-assistant.php（版本號 1.39.0 → 1.39.1）
- includes/modules/joint-burden/class-ur-ai-joint-burden-service.php
  （新增 calculate_condo_fund()，calculate() 支援自動／手動公共基金）
- includes/modules/joint-burden/class-ur-ai-joint-burden-ajax.php
  （新增 condo_fund_mode 參數）
- public/views/joint-burden-view.php、public/assets/js/joint-burden.js
- readme.txt
...
- PHP 服務層新增公共基金測試（8 項檢查點）：四個金額級距（500萬／
  5000萬／3億／15億）的累進提列手算對照、以及全鏈 auto／manual 兩種
  模式下金額與 auto 標記正確，全部通過。
- 既有第一階段 32 項、第二階段（含設計費、公共基金）38 項、全生命
  週期 23 項測試全部通過；`php -l`／`node --check` 通過。

六、是否建議上正式站

建議更新。純新增／強化功能，公共基金預設改為自動估算，可隨時切回
手動，並正確流入貸款利息 D 的扣除項，不影響其他計算。
