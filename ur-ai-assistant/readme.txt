...
= 1.39.1 =
* 「共同負擔提列估算」的「公寓大廈公共基金」新增自動估算：依公寓大廈管理條例施行細則第5條，以工程造價累進提列（1千萬內20‰、逾1千萬至1億15‰、逾1億至10億5‰、逾10億3‰）。條文所稱工程造價指建照載明造價，本工具以已估算之營建費用近似，實際仍以建照載明造價為準，亦可改回手動填入。

= 1.39.0 =
* 「共同負擔提列估算」的「建築設計費用」新增自動估算：依「全國建築師公會版建築師酬金標準表」（全建師會111字第0030號），以我們已算出的營建費用作為總工程費，按建築物種別（公共及高層／一般／特殊）與總工程費級距累進計算。因酬金費率為可協議的區間（各級距下限～9.0%），系統同時算出「下限累進金額」與「上限9%金額」，共同負擔小計採下限概算並在明細顯示區間。都更集合住宅預設「公共及高層建築」種別，亦可切換種別或改回手動填入。※ 此為全國建築師公會版本，非新北市建築師公會自訂版本，實際新北市個案仍應以新北市建築師公會酬金表為準。

= 1.38.1 =
* 「共同負擔提列估算」強化提醒：前台新增明顯的紅色警語橫幅，並在試算結果區（含友善列印）加上提醒——本估算僅供事前概算參考，實際金額會因個案條件（營建單價調整、各項個案認定費用、審查認定範圍等）而有差異，最終一律以「都市更新及爭議處理審議會」審查委員審議通過之數額為準。免責聲明預設文字亦同步以此提醒開頭。

= 1.38.0 =
* 「都市更新共同負擔提列估算」完成第二階段，補齊 A～H 完整結構：新增稅捐E（印花稅概估＋營業稅）、銷售管理費F4、容積獎勵維護費B，並可計算「共同負擔比率」。營業稅依財政部109年令釋公式計算——因公式中的「共同負擔」本身包含營業稅，屬循環定義，改以代數求出封閉解，並提供「房屋評定比例法／費用比例法」兩種擇一計算方式。填入「更新後總權利價值」後即算出營業稅與共同負擔比率；未填則結果僅到「不含營業稅」的部分。F4銷售管理費依獲配總價值分級累進（25億以下6%、25-50億5.5%、50億以上5%）。仍僅適用新北市提列基準，臺北市及其他縣市不適用。

= 1.37.0 =
* 新增「都市更新共同負擔提列估算」功能（第一階段，依「新北市」提列基準）：前台 shortcode `[ur_ai_joint_burden]`，輸入基地、樓層、面積、戶數、權屬等條件，概算共同負擔中有明確公式的項目——工程費用A（拆除、營建、外接管線）、權利變換費用C（規劃費、估價費、鑑界、鑽探、地籍整理）、貸款利息D、管理費用F（行政作業費F1、人事行政管理費F3、風險管理費F5），並附完整逐項公式明細、可友善列印。個案認定項目（建築設計費、公共設施、拆遷補償、信託費、都市計畫變更負擔G、容積移轉費H等）做成選填欄位。單價表、費率與級距為新北市公告基準，不開放後台調整；後台「共同負擔估算」頁可設定啟用狀態與免責聲明。※ 第一階段尚未計入稅捐E（含營業稅）、銷售管理費F4與共同負擔比率（需主管機關核定之更新後總權利價值），將於後續版本補上；臺北市及其他縣市之公式與費率不同，不適用本結果。
